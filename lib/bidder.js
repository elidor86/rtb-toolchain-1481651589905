...
var Bidder = function (req, res, next) {
    ...
    this.SanityCheck_MAX_DAILY_BIDs = 90000;
...
Bidder.prototype.process = function (err, campaign) {
    ...
    co(function *() {
        ...
        Elasticsearch.incCountersForBidSent({
...
Bidder.prototype.getCampaigns = function () {
    ...
    if (blackGeo.indexOf(self.bidReqData.GEO) > -1) {
        //global.logger.debug("blackGeo ");
        self.reason = 'blackGeo';
        return self.doNoBid();
    }
    ...
    if ((self.bidReqData.Publisher_ID in PublisherConfig) == false) {
        self.PublisherConfig.bidOutFormat = 'json';
        return self.doNoBid();
    }
...
Bidder.prototype.populateBidReqData = function () {
    ...
    try {
        ...
        bidReqData.Query = bidReqData.Query.split(",").join(" ").split("+").join(" ").replace(/\s\s+/g, ' ').toLowerCase();
    } catch (e) {
        //console.trace("  decodeURIComponent(data.keywords)  e ", e);
        bidReqData.Query = data.keywords;
    }
    ...
    self.PublisherConfig = PublisherConfig[bidReqData.Publisher_ID] || {};
...
Bidder.prototype.doBid = function () {
    ...
    if (self.PublisherConfig.bidOutFormat == 'json') {
    ...
    } else if (self.PublisherConfig.bidOutFormat == 'xml') {
        var url = self.bidReqData.BID_url.replace("&", '&amp;');
        var bidXml = '<?xml version="1.0" encoding="UTF-8"?><result status="BID"> <listing bid="' + campaign.BID_Value + '" url="' + url + '"/></result>';
        self.res.setHeader('Content-Type', 'text/xml');
        self.res.end(bidXml);
    }
...
Bidder.prototype.doNoBid = function () {
    ...
    if (self.PublisherConfig.bidOutFormat == 'json') {
        ...
        var bidXml = '<?xml version="1.0" encoding="UTF-8"?> <result status="NOBID"> </result>';
        self.res.setHeader('Content-Type', 'text/xml');
        self.res.end(bidXml);
    }
...
Bidder.prototype.makeReqToAws = function () {

    var self = this;


    var url = self.req.url;
    var awsHostname = 'http://rtb-adworks.us-west-2.elasticbeanstalk.com';

    var finalUrl = awsHostname + url;
    request.get(finalUrl, function (err, res, body) {
        // console.log("err ", err);
        // console.log("res ", res);
