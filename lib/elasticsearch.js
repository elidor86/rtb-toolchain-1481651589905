var elasticsearch = require('elasticsearch');
request = require('request');

var RateLimiter = require('limiter').RateLimiter;
var limiter = new RateLimiter(600, 'second');

var http = require('http');
var Agent = require('agentkeepalive');

var keepaliveAgent = new Agent({
    maxSockets: 5000,
    maxFreeSockets: 1000,
    timeout: 2000,
    keepAliveTimeout: 30000 // free socket keepalive for 30 seconds
});


var BidsToFlash = 2000;
//var BidsToFlash = 0;


//options.maxSockets = 4000;
//options.minSockets = 100;

/*
 if (process.env.DEV == 'true') {
 var esUrl = '104.196.118.159';
 var options = {
 host: '104.196.118.159:9200'
 };
 }*/

options = {
    host: 'http://ec2-35-165-214-118.us-west-2.compute.amazonaws.com:9200',
    pingTimeout: 10000,
    keepAlive: false
};


esUrl = 'http://ec2-35-165-214-118.us-west-2.compute.amazonaws.com';


var client = new elasticsearch.Client(options);


var esJobClient = new elasticsearch.Client({
    host: 'http://ec2-35-165-214-118.us-west-2.compute.amazonaws.com:9200',
    maxSockets: 5000,
    minSockets: 1000
});


var esBidsClient = new elasticsearch.Client({
    host: '104.154.21.195:9200',
    maxSockets: 5000,
    minSockets: 1000
});


/*
 client.create({
 index: 'rtb',
 type: 'campaigns',

 body: {
 "Channel_ID": 2,
 "Campaign_ID": 232,
 "Campaign_Name": "vhm-us",
 "BID_Value": 0.03,
 "Redirect_URL": "https://target-talent.com/campaigns/jobs/vhm",
 "Max_Num_Of_BIDs": 100,
 "Bid_URL": "http://rtb.dans-leads.com/tracking?Campaign_ID=@@Campaign_ID@@&BID_ID=@@BID_ID@@",
 "Filter_GEO": "(us)",
 "Filter_URLs": "()",
 "Filter_KeyWords": "()",
 "Filter_Function": "vhm",
 "Num_Of_BIDs_Sent": 0,
 "Num_Of_BIDS_Won": 0
 }
 }, function (error, response) {

 console.log("error ", error);
 console.log("response ", response);
 });*/


var Elasticsearch = {};

Elasticsearch.keepaliveAgent = keepaliveAgent;

Elasticsearch.client = client;

Elasticsearch.logBid = function (data) {

    //return;
    return new Promise(function (fulfill, reject) {

        client.create({
            index: 'bids',
            type: 'BIDs_Sent',
            body: data
        }, function (error, response) {
          ///  console.log("error ", error);
          //  console.log("response ", response);
            fulfill();

        }, function (err) {
            console.trace("Elasticsearch.logBid  ", err);
        });


    });
};

Elasticsearch.logEmail = function (data) {

    //return;
    return new Promise(function (fulfill, reject) {

        client.create({
            index: 'bids',
            type: 'emails',
            body: data
        }, function (error, response) {

            fulfill();
            // console.log("error ",error);
            // console.log("response ",response);
        });
    });

};

var noNids = [];

Elasticsearch.logNoBid = function (data) {

    return;

    noNids.push(data);
    //console.log("noNids", noNids.length);

    var flashToElastic = function (jobsToFlash) {
        var ops = [];

        for (var i = 0; i < jobsToFlash.length; i++) {

            var job = jobsToFlash[i];

            var operation = {
                create: {_index: 'bids', _type: 'BIDs'}
            };

            ops.push(operation);
            ops.push(job);
        }

        esBidsClient.bulk({
            body: ops
        }).then(function (resp) {
            // noop

        }, function (rejection) {
            console.log(('>>> ' + (new Date()).toUTCString()));
            console.log('' + JSON.stringify(rejection));
            console.log('<<<');
        });


    };

    if (noNids.length >= 3000) {
        flashToElastic(noNids.splice(0, 3000));
    }
};

Elasticsearch.incCountersForBidSent = function (data) {

    return new Promise(function (fulfill, reject) {

        client.bulk({
            body: [

                {update: {_index: 'rtb', _type: 'campaigns', _id: data.campaignId, '_retry_on_conflict': 500}},
                {script: 'ctx._source.Num_Of_BIDs_Sent += 1', upsert: {counter: 1}},

                {update: {_index: 'rtb', _type: 'dailyStat', _id: data.date, '_retry_on_conflict': 500}},
                {script: 'ctx._source.counter += 1', upsert: {counter: 1}}
            ]
        }, function (err, resp) {
            if (err || !resp || resp.errors == true) {
                console.trace("incCountersForBidSent err", err);
                console.trace("incCountersForBidSent resp", JSON.stringify(resp));
                return reject();
            }
            fulfill();
        });
    });

};

Elasticsearch.incCountersForBidWon = function (data) {

    return new Promise(function (fulfill, reject) {

        client.bulk({
            body: [

                {update: {_index: 'rtb', _type: 'campaigns', _id: data.Cmp_Es_ID}},
                {script: 'ctx._source.Num_Of_BIDS_Won += 1', upsert: {counter: 1}, retryOnConflict: 10}

            ]
        }, function (err, resp) {
            if (err || !resp || resp.errors == true) {
                console.trace("incCountersForBidSent err", err);
                console.trace("incCountersForBidSent resp", JSON.stringify(resp));
                return reject();
            }
            fulfill();
        });
    });

};

Elasticsearch.getDailyNumOfBid = function (data) {

    //console.log('getDailyNumOfBid data',data);
    return new Promise(function (fulfill, reject) {

        Elasticsearch.client.get({
            index: 'rtb',
            type: 'dailyStat',
            id: data
        }).then(function (resp) {

            var count = resp._source.counter;
            fulfill(count);


        }, function (err) {

            if (err && err.displayName == 'NotFound') {
                fulfill(0);
            } else {
                console.trace("displayName ", err);
                reject(err);
            }

        });
    });

};

Elasticsearch.getCmpById = function (id) {

    //console.log('getDailyNumOfBid data',data);
    return new Promise(function (fulfill, reject) {


        client.get({
            index: 'rtb',
            type: 'campaigns',
            id: id
        }, function (error, response) {

            ///return reject();
            if (error || !response) {
                reject();
            }
            // console.log('getCmpById error', error);
            // console.log('getCmpById response', response._source);

            fulfill(response._source);
        });


    });

};


Elasticsearch.getBidData = function (bidId) {


    return new Promise(function (fulfill, reject) {

        Elasticsearch.client.search({
            index: 'bids',
            type: 'BIDs_Sent',
            body: {
                "query": {
                    "match": {
                        "BID_ID": {
                            "query": bidId,
                            "type": "phrase"
                        }
                    }
                }
            }

            /*{

             "query": {
             "constant_score": {
             "filter": {
             "term": {
             "BID_ID": bidId
             }
             }
             }
             }
             }*/
        }).then(function (resp) {
           // console.log('getBidData bidId', bidId);

            try {
                var bidData = resp.hits.hits[0]._source;
                fulfill(bidData);
            } catch (e) {
                console.trace('getBidData err', e);
                console.log('getBidData resp', resp);
                return fulfill(null);
            }


        }, function (err) {

            console.trace('getBidData err', err);
            reject(err);


        });
    });

};

Elasticsearch.isBidInBidsWon = function (bidId) {

    //console.log('getBidData bidId', bidId);
    return new Promise(function (fulfill, reject) {

        Elasticsearch.client.search({
            index: 'bids',
            limit: 1,
            type: 'BIDs_Won',
            body: {
                "query": {
                    "constant_score": {
                        "filter": {
                            "term": {
                                "BID_ID": bidId
                            }
                        }
                    }
                }
            }
        }).then(function (resp) {

            // console.log('isBidInBidsWon ', resp);
            if (resp && resp.hits && resp.hits.hits && resp.hits.hits.length > 0) {
                fulfill(true);
            } else {
                fulfill(false);
            }


        }, function (err) {

            console.log('getBidData err', err);

            reject(err);

        });
    });

};


Elasticsearch.getBuyingList = function (name) {

    //console.log('getBuyingList name', name);
    var self = this;
    return new Promise(function (fulfill, reject) {

        Elasticsearch.client.search({
            index: 'bidding',
            type: name,
            body: {size: 2000}
        }).then(function (resp) {


            var data = resp.hits.hits;
            //console.log('getBuyingList data', data);
            var list = [];
            var removeItems = [];
            var now = new Date().getTime();
            data.forEach(function (item) {
                if (item._source.date) {
                    var diff = now - item._source.date;
                    if (diff < 1000 * 60 * 60) {
                        //if (diff < 1000) {
                        list.push(item._source.ip);
                    } else {
                        removeItems.push({
                            type: item._type,
                            id: item._id
                        })
                    }
                } else {
                    list.push(item._source.ip);
                }

            });
            // console.log('getBuyingList list', list);
            fulfill(list);

            if (removeItems.length > 0) {
                self.multiDelete(removeItems);
            }

        }, function (err) {

            console.log('getBidData err', err);

            reject(err);

        });
    });

};

Elasticsearch.multiDelete = function (items) {


    var ops = [];

    //console.log('multiDelete ', items);

    items.forEach(function (item) {
        ops.push({delete: {_index: 'bidding', _type: item.type, _id: item.id}})
    });

    return new Promise(function (fulfill, reject) {

        client.bulk({
            body: ops
        }, function (err, resp) {
            // console.log('deleteJobSeekers err', err);
            //console.log('deleteJobSeekers resp', resp);
            fulfill();
        });
    });

};


Elasticsearch.addToBuyingList = function (name, data) {

    //console.log('getBuyingList name', name);
    return new Promise(function (fulfill, reject) {

        client.create({
            index: 'bidding',
            type: name,
            id: data.ip,
            body: data
        }, function (error, response) {

            fulfill();
            // console.log("error ",error);
            // console.log("response ",response);
        });

    });

};


Elasticsearch.fetchEs = function (post_data, ip, path, cb) {


    post_data = JSON.stringify(post_data);
    var options = {
        host: esUrl,
        port: 9200,
        path: path,
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'Content-Length': Buffer.byteLength(post_data)
        },
        agent: keepaliveAgent
    };

    console.log("options ", options);

    var req = http.request(options, function (res) {

        if (res.statusCode != 200) {

            global.logger.debug('res.statusCode  err', res.statusCode);
            return cb({e: "statusCode " + res.statusCode}, null);
        }

        res.setEncoding('utf8');

        var body = '';
        res.on('data', function (chunk) {
            body += chunk;
            //global.logger.debug('body ', body);
        });

        res.on('end', function () {


            try {
                var resp = JSON.parse(body);
            } catch (e) {
                return cb(e, null);
            }

            //global.logger.debug('resp ', resp);
            return cb(null, resp);
        });
    });

    req.on('error', function (e) {
        return cb(e, null);
    });

    req.write(post_data);

    req.end();

};

Elasticsearch.getAllCmp = function () {

    var self = this;
    var start = new Date().getTime();

    return new Promise(function (fulfill, reject) {

        var body = {
            "size": 1000,
            "query": {
                "filtered": {
                    "filter": {
                        "script": {
                            "script": "doc[\"Num_Of_BIDs_Sent\"].value < doc[\"Max_Num_Of_BIDs\"].value"
                        }
                    },
                    "query": {
                        "match_all": {}
                    }
                }
            }
        };


        client.search({
            index: 'rtb',
            type: "campaigns",
            body: body
        }).then(function (resp) {


            var end = new Date().getTime();
            var diff = end - start;
            // console.log("time to get camp network ", JSON.stringify(resp));
            //global.logger.debug('getAllCmp err', err);


            try {
                var hits = resp.hits.hits;
                var campaigns = [];


                hits.forEach(function (item) {
                    // console.log("item ", item);
                    item._source._id = item._id;
                    campaigns.push(item._source);
                });


                //console.log("campaigns ", campaigns.length);
                //console.log("took ", resp.took);

                fulfill({campaigns: campaigns, took: resp.took, tookIncludeNetwork: diff});
            } catch (e) {
                console.log("fetchEs e ", e);
                fulfill({campaigns: [], took: 0, tookIncludeNetwork: 0});
            }


        }, function (err) {

            console.trace("fetchEs ee ", err);
            return reject();

        });


    });


};

Elasticsearch.getJobUrl = function (data) {

    var self = this;

    global.logger.debug('getJobUrl data ', data);
    return new Promise(function (fulfill, reject) {


        var should = [];

        should.push({"match_phrase": {"title": {"query": data.q, "boost": 5, "slop": 5}}});
        should.push({"common": {"title.english": {"cutoff_frequency": 0.001, "boost": 1000, "query": data.q}}});

        var query = {
            "bool": {
                "must": {
                    "bool": {
                        "should": should
                    }
                }
            }
        };


        var body = {
            "size": 1,
            "sort": {"ranking": {"order": "asc"}},
            "query": {
                "filtered": {
                    "query": query,
                    "filter": {
                        "geo_distance": {
                            "distance": 10 + "km",
                            "latlon": {
                                "lat": data.lat,
                                "lon": data.lon
                            }
                        }
                    }
                }
            }
        };

        //console.log("body ", JSON.stringify(body));


        esJobClient.search({
            index: 'jobs',
            _source: "url",
            body: body
        }).then(function (resp) {


            try {
                var hits = resp.hits.hits;


                if (hits[0] && hits[0]._source) {
                    var jobUrl = hits[0]._source.url;
                    global.logger.debug('hits[0]._source ', hits[0]._source);
                    fulfill(jobUrl);
                } else {
                    fulfill();
                }


                // global.logger.debug('jobUrl ', jobUrl);
                //console.log("jobUrl ", jobUrl);
                //console.log("took ", resp.took);


            } catch (e) {
                console.trace("getJobUrl e ", e);
                reject(e);
            }


        }, function (err) {

            if (err && err.message && err.message.search("No mapping found") > -1) {

            } else {
                console.trace("fetchEs ee ", err);
            }

            return reject();

        });

    });


};


module.exports = Elasticsearch;