...
var Campaigns = function (data, cb) {
    ...
    this.baseShortUrl = (process.env.DEV == 'true' ? 'https://rtb.ngrok.io/redirect/' : 'http://rtb.dans-leads.com/redirect/' );
    this.MIN_INTERVAL = 1000 * 60 * 60 * 24;
...
/*
 ...
 BID_Value: 0.03,
 Redirect_URL: 'http://target-talent.com/campaigns/jobs/vhm',
 Max_Num_Of_BIDs: 999999,
 Bid_URL: 'https://rtb.ngrok.io/tracking?Campaign_ID=@@Campaign_ID@@&BID_ID=@@BID_ID@@',
 Filter_GEO: '(us)',
 Filter_URLs: '()',
 Filter_KeyWords: '()',
 Filter_Function: 'vhm',
 Num_Of_BIDs_Sent: 11265,
 MIN_INTERVAL: 1000 * 60,
 Num_Of_BIDS_Won: 243336,
 counter: '',
 BuyingList: '',
 sub_id: 'C232',
 network: 'vhm',
 Max_Imp: 2,
 _id: 'AVURoXNWmvYfB07zPZ1R'
 }
 ]
 }*/
...
Campaigns.prototype.selectCampaign = function () {
    ...
    co(function *() {
        ...
        for (var i = 0; i < campaigns.length; i++) {
            ...
            // console.log("\n\n\n\n\n\n\n ");
            if (filterByPublisher == true && filterByKeywords == true && filterByUrl == true && filterByGeo == true && filterByFunction == true) {
...
Campaigns.prototype.impressionsCheck = function (campaign) {
    ...
    return new Promise(function (fulfill, reject) {
        co(function *() {
            ...
            //console.log("userImpressionsPerCmp  ", userImpressionsPerCmp);


            if (!userImpressionsPerCmp || typeof userImpressionsPerCmp.count != 'number') {
                return fulfill(false);
            }

            if (userImpressionsPerCmp.count == 0) {
                return fulfill(true);
            }


            if (userImpressionsPerCmp.count < maxImpPerCmp) {

                var now = new Date().getTime();
                var LastImpTs = userImpressionsPerCmp.LastImpTs;
                var diff = now - LastImpTs;

                var MIN_INTERVAL = campaign.MIN_INTERVAL || self.MIN_INTERVAL;
...
Campaigns.prototype.vhm = function () {
    ...
    return new Promise(function (fulfill, reject) {
        co(function *() {
            ...
            //console.log("self.bidReqData ", self.bidReqData);
            var bidReqData = self.bidReqData;
            var predict = Classifier.predictFromUrl(bidReqData.URL);
            ...
            if (jobUrl && jobUrl.length > 0) {
                ...
                var url = jobUrl + "&subid=C232_" + bidReqData.GEO;
                var uid = shortid.generate();

                memcached.createRedirect({
                    url: url,
                    uid: uid
                });

                var shortUrl = self.baseShortUrl + uid;
