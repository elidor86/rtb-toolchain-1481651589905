...
var Bidder = function (req, res, next) {
    ...
    //global.logger.debug('req.query', req.query);
    //global.logger.debug('req.query', req.url);
    global.logger.debug('req.query', req.params.id);
...
Bidder.prototype.populateBidReqData = function () {
    ...
    bidReqData.Referrer = data.referer || '';
...
Bidder.prototype.process = function (err, campaign) {
    ...
    co(function *() {
        ...
        console.log("SanityCheck ", SanityCheck);
...
Bidder.prototype.SanityCheck = function () {
    ...
    return new Promise(function (fulfill, reject) {
        ...
        co(function *() {
            ...
            if (self.campaign.BID_Value > 0.02) {
                self.reason = 'BID_Value_High';
                fulfill([false]);
            }

            if (!self.bidReqData.URL || self.bidReqData.URL.length < 7) {
            ...
            console.log("_DailyNumOfBid ", _DailyNumOfBid);

            if (_DailyNumOfBid && _DailyNumOfBid.count >= 200) {
                fulfill([false]);
            }
